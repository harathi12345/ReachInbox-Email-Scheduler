...
const Analytics = () => {
  ...
  const statItems = [
    ...
    { label: "Sent Campaigns", value: stats.sent, icon: Send, color: "green" },
    { label: "Scheduled Campaigns", value: stats.scheduled, icon: Clock3, color: "purple" },
    { label: "Failed Campaigns", value: stats.failed, icon: XCircle, color: "rose" },
    { label: "Cancelled Campaigns", value: stats.cancelled, icon: Ban, color: "amber" },
  ];

  return <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
    <motion.section className="mb-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-end" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}><div><p className="section-label">Performance overview</p><h2 className="mt-2 text-2xl font-bold tracking-[-0.04em] text-white sm:text-[30px]">Analytics</h2><p className="mt-2 text-sm text-slate-500">Track real campaign delivery data.</p></div><div className="flex rounded-lg border border-white/[0.08] bg-white/[0.03] p-1">{ranges.map((item) => <button key={item.days} className={`rounded-md px-3 py-2 text-xs font-semibold transition-colors ${range === item.days ? "bg-blue-500/15 text-blue-300" : "text-slate-500 hover:text-slate-300"}`} onClick={() => setRange(item.days)}>{item.label}</button>)}</div></motion.section>
